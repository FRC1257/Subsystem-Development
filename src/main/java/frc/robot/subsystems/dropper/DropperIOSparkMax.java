...
public class DropperIOSparkMax implements DropperIO {
  ...
  public DropperIOSparkMax() {
    ...
    leftMotor =
        new SparkMax(
            DropperConstants.LEFT_DROPPER_MOTOR_ID, MotorType.kBrushless); // ask the type of motor
  ...
  public void updateInputs(DropperIOInputs inputs) { // update the imputs form io
    ...
    inputs.angVelocityRadsPerSec =
        leftEncoder.getVelocity() * 2 * pi / 60; // in rotations so radians per sec converted
  ...
  public double getAngle() {
    return leftEncoder.getPosition()
        * 2
        * pi; // its just gets the rotations and convers to angle using two pi make a pi constant
    // later
  }
    ...
    double acceleration =
        (pidController.getSetpoint().velocity - lastSpeed)
            / (Timer.getFPGATimestamp()
                - lastTime); // change in velocity over change in time... physics..
